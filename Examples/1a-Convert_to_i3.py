...
def make_i3_particle(particle_data):
    ...
    if (
        abs(pdgcode) >= 12 and abs(pdgcode) <= 16
    ):  # 12-16 is nu_e, mu, n_mu, tau, nu_tau
...
class li_parser(icetray.I3Module):
    ...
    def DAQ(self, frame):
        ...
        dataclasses.I3MCTree.add_primary(
            frame["I3MCTree"], make_i3_particle(active["initial"][self.event_no])
        )
        dataclasses.I3MCTree.append_child(
            frame["I3MCTree"],
            frame["I3MCTree"][0].id,
            make_i3_particle(active["final_1"][self.event_no]),
        )
        dataclasses.I3MCTree.append_child(
            frame["I3MCTree"],
            frame["I3MCTree"][0].id,
            make_i3_particle(active["final_2"][self.event_no]),
        )

        frame["I3EventHeader"] = dataclasses.I3EventHeader()
        frame["EventProperties"] = get_prop_dict(
            active["properties"][self.event_no], "Ranged" in self.keys[self.keyno]
        )
