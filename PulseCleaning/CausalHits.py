...
class CausalPulseCleaning(icetray.I3ConditionalModule):
    ...
    def __init__(self, context):
        ...
        self.AddParameter(
            "windowscale",
            "Scale the causality window by factor to make it a looser cut",
            1.2,
        )
        self.AddParameter(
            "maxphotonatten",
            "Maximum number of attenuationlengths photons can travel",
            2.3,
        )
        self.AddParameter(
            "MaxTracktoReferenceDOM",
            "Maximum distance between Reference DOM and track",
            50.0,
        )
    ...
    def getBaseTimeandPosition(self, mcpeMap):
        ...
        for PMTKey in mcpeMap.keys():
            if (PMTKey.string == MaxchargeDOM.string) and (
                PMTKey.om == MaxchargeDOM.om
            ):
        ...
        for ipulse1 in range(len(pulses)):
            ...
            for ipulse2 in range(len(pulses)):
                if abs(pulses[ipulse1] - pulses[ipulse2]) < 10.0:
                    pulsecoinc[-1] += 1
    ...
    def getCausalMCPEs(self, pos1, pos2, mcpeList, coincetime):
        ...
        if dis_maxphoton < dis:
            dis_part = (
                np.sin(
                    theta_c
                    - np.arcsin(
                        np.sin(np.pi - theta_c)
                        * (self.attenlength * self.maxphotonatten / dis)
                    )
                )
                * dis
                / np.sin(np.pi - theta_c)
        ...
        WindowMin = (
            max(0.0, (dis / c) - self.MaxTracktoReferenceDOM * ngroup / c)
            / self.windowscale
        )
