    ...
    def getValue(self, inputVal):
        ...
        for i in range(len(self.outputs)):
            if inputVal <= self.inputs[i + 1]:
                index = i
                break
        fraction = (inputVal - self.inputs[index]) / (
            self.inputs[index + 1] - self.inputs[index]
        )

        return (
            self.outputs[index]
            + (self.outputs[index + 1] - self.outputs[index]) * fraction
        )
...
class Polynomial:
    ...
    def normalize(self):
        ...
        self.coefficients = [
            coefficient / integral for coefficient in self.coefficients
        ]
