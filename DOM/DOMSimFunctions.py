...
def getSurvivalProbability(
    geoMap, photon, omkey, angularAcceptance, domAcceptance, relativeEfficieny
):
    domGeo = geoMap[omkey]
    domDirection = domGeo.direction
    photonDirection = photon.dir
    dotProduct = (
        photonDirection.x * domDirection.x
        + photonDirection.y * domDirection.y
        + photonDirection.z * domDirection.z
    )
...
def survived(
    ...
    probability = getSurvivalProbability(
        geoMap, photon, omkey, angularAcceptance, domAcceptance, relativeEfficieny
    )
    ...
    if probability > 1:
        raise ValueError(
            "Probability = "
            + str(probability)
            + " > 1. Most likely photon weights are too high"
        )
...
def generateMCPEList(
    geoMap, photons, modkey, angularAcceptance, domAcceptance, relativeEfficieny
):
    ...
    for photon in photons:
        if survived(
            ...
            mcpe.time = (
                photon.time
            )  # + np.random.normal(0, 1.5) #smearing the time with random values from a gaussian with width 1.5ns
...
def selectStartTime(timestamps):
    ...
    noiseWindow = timestamps[
        (timestamps >= randStartTime) & (timestamps <= randStartTime + 72e2)
    ]
...
def generateRecoPulses(omkey, hits):
    ...
    for mcpe in hits:
        ...
        recoPulse.time = mcpe.time + np.random.normal(
            0, 1.5
        )  # smearing the time with random values from a gaussian with width 1.5ns
