...
class SimpleDOMSimulation(icetray.I3ConditionalModule):
    ...
    def ApplyPMTCharacteristics(self,mcpemap):
        ...
        if len(pulseoutoftimelist) > 0 :
            ...
            pulsetimelist = self.CombineOrderedLists(pulsetimelist.copy(),pulseoutoftimelist)
    ...
    def MakeRecoPulses(self,pulsetimelist,pulsechargelist,omkey,outputpulsemap,DOMpulsemap=None) :
        mingap = 4.0
        minindex = -1
        ...
        if len(pulsetimelist) > 100 :
            ...
            while following < len(pulsetimelist):
                if (pulsetimelist[following][0]-pulsetimelist[leading][0]) < 3.0 and pulsechargelist[leading]*pulsechargelist[following] > 0.0:
            ...
            for iom in range(1,self.nom+1) :
                ...
                if omkey in mcpemap.keys():
                    ...
                    if omkey in darkhits.keys():
                        DarkPulses = self.ApplyPMTCharacteristicsDarkNoise(darkhits[omkey])
                        #print("dark")
    ...
    def DAQ(self,frame) :
        ...
        frame[self.outputmap], frame[self.outputmap+"_nonoise"], frame['triggerpulsemap'] = self.ApplyPMTResponse(photonmap_on_pmts,darkhits)
