...
class WaveformBuilder(icetray.I3ConditionalModule):
    ...
    def __init__(self, context):
        ...
        self.AddParameter(
            "inputmap", "Name of the Physics I3MCTree name", "I3MCPulseSeriesMap"
        )
        self.AddParameter(
            "outputmap", "Name of the noise I3MCTree name", "I3RecoPulseSeriesMap"
        )
    ...
    def DAQ(self, frame):
        ...
        for omkey in mcpulseOMKeys:
            ...
            binwidth = 4.0
            tmin = min(pulsetimes) - 300.0
            tmax = max(pulsetimes) + 300.0
            ...
            for i in range(len(waveform) - 10):
                if inwaveform:
                    if (
                        abs(waveform[i]) < 4
                        and abs(waveform[i + 1]) < 4
                        and abs(waveform[i + 2]) < 4
                        and abs(waveform[i + 3]) < 4
                    ):
                        for j in range(10):
