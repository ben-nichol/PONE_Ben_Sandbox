...
def GetDOMQuantumEfficiency():
    ...
    startVal = 300.0 * I3Units.nanometer
    steps = 10.0 * I3Units.nanometer
...
def GetDOMAcceptance(domRadius=0.2159 * I3Units.m):  # 17 inch diameter
    ...
    for wavelength in range(300, 611, 10):
        ...
        if (this_abs_glass <= 0.0) or (this_abs_gel <= 0.0):
            current_om_eff_area = 0.0
        else:
            current_om_eff_area = (
                pmt_area
...
def GetIceCubeDOMAcceptance(
    domRadius=0.16510 * I3Units.m, efficiency=1.0, highQE=False, coverageFactor=1
):
    """
    ...
    dom2007a_eff_area = (
        np.array(dom2007a_eff_area) * I3Units.meter2
    )  # apply units (this is an effective area)
    domArea = np.pi * domRadius**2.0
    dom2007a_efficiency = efficiency * (dom2007a_eff_area / domArea)

    if highQE:
        wv, rde = np.loadtxt(
            expandvars("$I3_BUILD/ice-models/resources/models/wavelength/wv.rde")
        ).T
        dom2007a_efficiency *= np.interp(
            260 + 10 * np.arange(len(dom2007a_efficiency)), wv, rde
        )

    startVal = 260.0 * I3Units.nanometer
    steps = 10.0 * I3Units.nanometer
    inputs = [(startVal + i * steps) for i in range(len(dom2007a_efficiency))]

    return FunctionClasses.FunctionFromTable(
        inputs, dom2007a_efficiency * coverageFactor
    )
...
upgradeNumPMTs = 24
upgradeCoverage = (upgradeNumPMTs * np.pi * upgradePMTRadius**2) / (
    4 * np.pi * upgradeDOMRadius**2
)
...
domAcceptance = GetIceCubeDOMAcceptance(
    coverageFactor=upgradeCoverage / currentCoverage
)

aaFile.write(
    "# This file contains polynomial coefficients for DOM angular Acceptance\n"
)
...
deFile.write(
    "# This file contains input output values for the DOM efficiency distribution (eff vs. wavelength)\n"
)
