...
class MuonGenerator:
    """
    muon generator class header example, tobe flushed out with details of generator use
    """
    ...
    def ReadFluxTable(self):
        ...
        for i in range(1, len(zenithbinlines)):
            ...
            try:
                ...
                self.zenithbins.append(max(_flux * (np.pi / 180.0), 0.0))
        ...
        for l in range(Zenithline + 1, len(lines)):
            ...
            for i in range(1, len(splitline)):
                try:
                    ...
                    self.flux[i - 1].append(
                        max(_flux * np.cos(self.zenithbins[i - 1]), 0.0)
                    )
        ...
        self.de = (self.energybins[-1] - self.energybins[0]) / (
            len(self.energybins) - 1
        )
    ...
    def GetZenithCDF(self, injectR):
        ...
        for z in range(int(minZenith / dz), int(np.pi / dz)):
            ...
            ZenithWeight.append(
                rate * np.sin(self.minzenith + (0.5 + float(z)) * self.dz)
            )
        ...
        for i in range(self.d_cdf):
            ...
            while Zenithweight[z] / totalflux <= p:
                ...
                z += 1
            self.zenithcdf.append(
                self.minzenith
                + (float(z) + (p / (ZenithWeight[z] / totalflux))) * self.dz
            )
        ...
        for z in range(len(self.flux)):
            ...
            for i in range(self.d_cdf):
                ...
                while eflux[e] / totalflux <= p:
                    ...
                    e += 1
                inverse_cdf.append(
                    self.minenergy + (e + (p / (eflux[e] / totalflux))) * self.de
                )
            self.energyinvers_cdfs.append(inverse_cdf)

    def GetEnergyde(self, e):
        return 10.0 ** (self.minenergy + self.de * float(e + 1)) - 10.0 ** (
            self.minenergy + self.de * float(e)
        )
    ...
    def MakeZenithCDF(self):
        ...
        for z in range(len(self.flux)):
            ...
            for e in range(len(self.flux[z])):
                _flux += (
                    self.flux[z][e]
                    * self.GetEnergyde(e)
                    * np.sin(self.minzenith + (0.5 + float(z)) * self.dz)
                )
        ...
        for z in range(1, len(self.FullZenithCDF)):
            self.FullZenithCDF[z] = (
                self.FullZenithCDF[z - 1] + self.FullZenithCDF[z] / totalFlux
            )
    ...
    def GenerateRandomEvent(self):
        ...
        while radius > 6000.0:
            ...
            if rbin_int < len(self.cdf_inv) - 1:
                radius += (rbin - rbin_int) * (
                    self.cdf_inv[rbin_int + 1] - self.cdf_inv[rbin_int]
                )
        ...
        if (
            zenbin < len(self.FullZenithCDF) - 1
            and abs(minzenith - zenbin * self.dz) > 0.0
        ):
        ...
        if not p < 1.0:
            # print("p = "+str(p))
            p = 0.999999
        weight = (np.pi / maxtheta) * (1.0 / (1.0 - p))
        # weight = p
        random = np.random.rand()
        theta = (-1.0 + 2.0 * random) * maxtheta

        efluxbin = int(
            min(max(0, (energyexp - self.minenergy) / self.de), len(self.flux[0]) - 1)
        )
        zfluxbin = int(min(max(0, zenith / self.dz), len(self.flux) - 1))
        flux = self.flux[zfluxbin][efluxbin]

        vertex = [
            radius * np.cos(injecttheta),
            radius * np.sin(injecttheta),
            self.cylH / 2.0,
        ]
        direction = [
            -np.cos(injecttheta) * np.sin(zenith),
            -np.sin(injecttheta) * np.sin(zenith),
            -np.cos(zenith),
        ]
        direction_rotated = [
            direction[0] * np.cos(theta) - direction[1] * np.sin(theta),
