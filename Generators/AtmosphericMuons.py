...
class MuonGenerator(icetray.I3Module):
    ...
    def Configure(self):
        ...
        d = pd.read_csv(self.GetParameter("Fluxfile"), 
            comment="#",
        ...
        self.total_flux          = integrate.simps(
            angle_int*nn.sin(flux_angle_rad)*2*nn.pi,
            flux_angle_rad,
            axis=0) / I3Units.s / I3Units.cm2
    ...
    def DAQ(self, frame):
        ...
        for axis, angle in [("z", rand.uniform(0, 2 * nn.pi)),
                            ("y", angle),
        ...
        if not self.has_sframe:
            ...
            sframe.Put(prefix+"Area", dataclasses.I3Double(nn.pi*self.GetParameter("InjectionRadius")**2))
...
class MuonReweighter(MuonGenerator):
    ...
    def DAQ(self, frame):
        assert len(frame[self.GetParameter("InputI3MCTreeName")]) == 1, \
