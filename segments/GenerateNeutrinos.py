...
def GenerateNeutrinos(
    ...
    GammaIndex=2.0,
    FromEnergy=1.0 * I3Units.TeV,
    ToEnergy=10.0 * I3Units.PeV,
    ...
    tray.AddService(
        "I3EarthModelServiceFactory", earthModelService, **earthModelServiceArgs
    )
    ...
    if Flavor != "":
        ...
        tray.AddModule(
            "I3NuGDiffuseSource", injector, SteeringName=steering, **injectorServiceArgs
        )
...
def SelectNeutrino(
    ...
    for flavor in "E", "Mu", "Tau":
        for ptype in "", "Bar":
            Propagators[
                getattr(dataclasses.I3Particle.ParticleType, "Nu" + flavor + ptype)
            ] = (tau_prop if flavor == "Tau" else prop)


@icetray.traysegment
def GenerateAtmosphericNeutrinos(
    tray,
    name,
    Files,
    GCDFile="",
