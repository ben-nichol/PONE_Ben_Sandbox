...
def tasksetInUse():
    ...
    try:
        num_cpus = reduce(
            lambda b, a: b + int("processor" in a), open("/proc/cpuinfo").readlines(), 0
        )
...
def resetTasksetThreads(main_pid):
    ...
    try:
        ...
        tt = "1" * num_cpus
        p = subprocess.Popen(
            ["/bin/ps", "-Lo", "tid", "--no-headers", "%d" % main_pid],
...
def LoadCascadeTables(IceModel="SpiceMie", TablePath="/data/sim/sim-new/spline-tables"):
    ...
    cascade_service = photonics_service.I3PhotoSplineService(
        ...
        timingSigma=0.0,
        maxRadius=800.0 * icetray.I3Units.meter,
    )
...
def PropagatePhotons(
    ...
    if isinstance(IceModel, clsim.I3CLSimMediumProperties):
        ...
        for d in listdir(IceModelLocation):
            if (
                isdir(expandvars(IceModelLocation + "/" + d))
                and IceModel.lower() == d.lower()
            ):
    ...
    if HybridMode and IceModel not in ("Spice1", "SpiceMie"):
        raise RuntimeError(
            "Can only use Spice1 and SpiceMie in hybrid mode. photon tables do not support ice anisotropy at this time."
        )

    if (not IgnoreCascades) and HybridMode:
        if CascadeService is None:
            logging.warning(
                "*** no cascades tables provided. Loading tables for", IceModel
            )
            ...
            UseSplinesFromCVMFS = os.path.isdir(
            ...
            if HoleIceParameterization == expandvars(
                "$I3_SRC/ice-models/resources/models/angsens/as.h2-50cm"
            ):
                CascadeModel = IceModel
            elif HoleIceParameterization == expandvars(
                ...
                raise RuntimeError(
                    "No spline for %s with hole ice param %s",
                    IceModel,
                    HoleIceParameterization,
                )

            cascade_service = LoadCascadeTables(
                IceModel=CascadeModel, TablePath=TablePath
            )
        else:
    ...
    if HybridMode:
        ...
        if (CrossoverEnergyEM is not None) or (CrossoverEnergyHadron is not None):
            raise RuntimeError(
                "CrossoverEnergyEM or CrossoverEnergyHadron not supported in hybrid mode"
            )
        ...
        tray.AddModule(
            ...
            OutputMCTreeName=InputMCTree + "Tracks",
        )  # overwrite the input
        ...
        if not IgnoreMuons:
            if UseGPUs:
                ...
                threading.Thread(
                    target=resetTasksetThreads, args=(os.getpid(),)
                ).start()
            ...
            tray.AddModule(
                "Delete",
                name + "_cleanup_clsim_sliced_MCTree",
        ...
        if not KeepIndividualMaps:
            ...
            tray.AddModule(
                "Delete", name + "_cleanup_peseriesmaps", Keys=MCPEsToCombine
            )

            tray.AddModule(
                "Delete",
                name + "_cleanup_MCTree",
