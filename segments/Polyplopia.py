...
def PolyplopiaSegment(
    ...
    if bgfile:  # merge bg into signal
        ...
        outsurface_center = dataclasses.I3Position(
            cylinder_x * I3Units.m, cylinder_y * I3Units.m, cylinder_z * I3Units.m
        )
...
def PolyplopiaPhotons(
    ...
    if mctree_name != "SignalI3MCTree":
        ...
        def merge_trees(f):
            if not "SignalI3MCTree" in f or not "BackgroundI3MCTree" in f:
                raise Exception(
                    "Could not find either signal or background tree in frame!"
                )
    ...
    tray.AddModule(
        "Rename", "rename_mmc1", Keys=["MMCTrackList", "BackgroundMMCTrackList"]
    )
