...
@icetray.traysegment
def Calibration(tray, name, BadDOMsHLC=None, BadDOMsSLC=None):
        ...
        for key in staticList:
            if key not in dynamicList:
                icetray.logging.log_warn(
                    "key {} is not in dynamic (in-frame) BadDOMList".format(key)
                )
                retval = False
    ...
    def AddBadDOMLists(frame, listHLC, listSLC):
        if "BadDomsList" in frame:
            ...
            if not identicalHLC:
                ...
                icetray.logging.log_warn(
                    "  -> new DOM entries in updated list are: {}".format(
    ...
    tray.AddModule(
        createEmptyTimeWindows,
        name + "_createEmptyTimeWindows",
    ...
    tray.AddModule(
        convertToVectorOMKey,
        name + "_convertToVectorOMKey",
        Streams=[icetray.I3Frame.DAQ],
