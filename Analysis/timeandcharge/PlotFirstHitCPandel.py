...
def cpandel(t, d, sigma = 2.0, lambda_s = 120., rho = 0.004, t0=0.0):
...
def shadowfirstpulse():
	...
	for i in range(len(pdf)):
		...
		for j in range(len(pdf[i])):
			value = pdf[i][j]
			pdf[i][j] *= (1-partial_int/integral)
			partial_int += value
			if pdf[i][j] > maxvalue :
				maxvalue = pdf[i][j]
				peaktime[i] = j
...
if __name__ == "__main__":
	...
	sigma = 3.618178014267083
	lambda_s = 31.865107516246415
	rho = 0.02866231841555611
	t0 =  10.486423303198332

	sigma = 3.745618927674934
	lambda_s = 38.955158696610724
	rho = 0.01808837376642808
	t0 = 11.395377473038216

	sigma = 3.217943190145301
	lambda_s = 31.634096100829
	rho = 0.03671119404380607
	t0 = 10.169002096297923

	sigma = 3.0707618904243255
	lambda_s = 30.649218813580028
	rho = 0.04480410759637499
	t0 = 10.110187951499816

	fout = ROOT.TFile("FirstHitCPandelPlot.root","RECREATE")
	...
	amp = pdf[10][peaktime[10]]/cpandel(float(peaktime[10])-9.5, float(10)+0.5, sigma, lambda_s, rho,t0)
	for i in range(1,211) :
		MC_10_11.Fill(-10.0+i,pdf[10][i])
	for i in range(1,211) :
		CPandel_10_11.Fill(-10.0+i,amp*cpandel(-10.0+i,10.5,sigma,lambda_s,rho,t0))
	amp = pdf[20][peaktime[20]]/cpandel(float(peaktime[20])-9.5, float(20)+0.5, sigma, lambda_s, rho,t0)
	for i in range(1,211) :
                MC_20_21.Fill(-10.0+i,pdf[20][i])
        for i in range(1,211) :
                CPandel_20_21.Fill(-10.0+i,amp*cpandel(-10.0+i,20.5,sigma,lambda_s,rho,t0))
	amp = pdf[50][peaktime[50]]/cpandel(float(peaktime[50])-9.5, float(50)+0.5, sigma, lambda_s, rho,t0)
	for i in range(1,211) :
                MC_50_51.Fill(-10.0+i,pdf[50][i])
        for i in range(1,211) :
                CPandel_50_51.Fill(-10.0+i,amp*cpandel(-10.0+i,50.5,sigma,lambda_s,rho,t0))
	amp = pdf[100][peaktime[100]]/cpandel(float(peaktime[100])-9.5, float(100)+0.5, sigma, lambda_s, rho,t0)
	for i in range(1,211) :
                MC_100_101.Fill(-10.0+i,pdf[100][i])
        for i in range(1,211) :
                CPandel_100_101.Fill(-10.0+i,amp*cpandel(-10.0+i,100.5,sigma,lambda_s,rho,t0))
