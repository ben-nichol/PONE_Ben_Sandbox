...
#
#
# Need to include the parameter: UnWeightedPhotons=True
# in clsim script because the wavelength range in this
# file goes past the default acceptance range
#
...
def GetPetzoldScatteringCosAngleDistribution():
    ...
    petzold_data_val = [
        0.0,  # <- this 0 will be replaced right after the definition
        1.767e03,
        1.296e03,
        9.502e02,
        6.991e02,
    ...
    petzold_data_val[0] = (
        2.0
        * math.pi
        * math.sin(petzold_data_ang[1])
        * petzold_data_val[1]
        * (
            (petzold_data_ang[0] / petzold_data_ang[1])
            ** petzold_powerLawIndexBeforeFirstBin
        )
    )

    for i in range(len(petzold_data_val)):
        petzold_data_val[i] = (
            2.0 * math.pi * petzold_data_val[i] * math.sin(petzold_data_ang[i])
        )

    petzoldScatModelAngle = I3CLSimRandomValueInterpolatedDistribution(
        x=petzold_data_ang, y=petzold_data_val
    )
    petzoldScatModelCosAngle = I3CLSimRandomValueApplyFunction(
        petzoldScatModelAngle, "cos"
    )
...
def MakePoneMediumProperties():
    ...
    m = I3CLSimMediumProperties(
        mediumDensity=1.039 * I3Units.g / I3Units.cm3,
        layersNum=1,
        layersZStart=-310.0 * I3Units.m,
        layersHeight=2500.0 * I3Units.m,
        rockZCoordinate=-310.0 * I3Units.m,
        airZCoordinate=2500.0 * I3Units.m - 310.0 * I3Units.m,
    )
    ...
    m.ForcedMinWlen = 250.0 * I3Units.nanometer
    m.ForcedMaxWlen = 720.0 * I3Units.nanometer
    ...
    m.SetDirectionalAbsorptionLengthCorrection(I3CLSimScalarFieldConstant(1.0))
    ...
    # no "ice" tilt
    m.SetIceTiltZShift(I3CLSimScalarFieldConstant(0.0))
    ...
    ponePhaseRefIndex = I3CLSimFunctionRefIndexQuanFry(
        pressure=215.82225 * I3Units.bar,
        temperature=1.78,
        salinity=34.82 * I3Units.perThousand,
    )
    poneScattering = I3CLSimFunctionScatLenPartic(
        volumeConcentrationSmallParticles=0.0075 * I3Units.perMillion,
        volumeConcentrationLargeParticles=0.0075 * I3Units.perMillion,
    )

    # these are for P-ONE (linear interpolation between STRAW data points with extended tails)
    # absorption lengths from 260nm to 710nm in 10nm increments
    absLenTable = [
        0.800,
        1.629,
        2.457,
    ...
    poneAbsorption = I3CLSimFunctionFromTable(
        260.0 * I3Units.nanometer, 10.0 * I3Units.nanometer, absLenTable
    )
