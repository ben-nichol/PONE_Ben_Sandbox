...
averageAngle = (
    str((1 - eda) * 0.924) + "\t# g=<cos(theta)> (formula from ANTARES paper)\n"
)
...
alpha = (
    ...
    + "\t"
    + str(0)
    + "\t# scattering wavelength dependence power law exponent\n"
)
kappa = (
    str(-absLSS[0])
    + "\t"
    + str(0)
    + "\t# absorption wavelength dependence power law exponent\n"
)
...
B = (
    ...
    + "\t"
    + str(0)
    + "\t# absorption exponential component exponent multiple - ignored\n"
)
...
for i in range(num_elements):
    line = (
        str(depth[i]) + " " + str(b_e[i]) + " " + str(a_e[i]) + " " + str(dT[i]) + "\n"
    )
    outfile.write(line)
