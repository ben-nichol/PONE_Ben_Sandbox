...
def getMeanEventRate(s = 3.482, frameLen = 10000):
    ...
    totFrameEvents = Bq * (4./3.) * np.pi * (R**3)
...
class myQuaternion:
    ...
    def printQ(self):
        print('%.3f + %.3fi + %.3fj + %.3fk'%(self.s,self.u[0],self.u[1],self.u[2]))
...
class K40:
    ...
    def __init__(self, dD, sP, nP, bN, sD, pD, eAD, wls, aDv, sID, tDiff):
        ...
        self.nPProbs = nP        # Number of photon probability as a function of distance [50][10]
        self.bigHitN = bN        # Number of photons distributions for events with >6 photons [4][40]
        ...
        self.tDiffs = tDiff      # Arrival time difference distribution [4][...] for each scattering
    ...
    def getD(self, num):
        ...
        for i in np.arange(num):
            dIndex = np.random.choice(np.arange(len(self.dBCentres)), 1, p=self.dDist)[0]
    ...
    def getNumPs(self, ds, num):
        ...
        for i in np.arange(num):
            ...
            if nPs > 6:
                nPs = int(np.random.choice(self.bigHitBinCentres, 1, p=self.bigHitN[nPs-7])[0])
            nPArr[i] = nPs
    ...
    def getLinPoints(self, nPs):
        ...
        # Get second point
        ang = self.smear(np.random.choice(self.linAngBinCentres,
                                                      1, p=self.eventAngDist[ind])[0], self.linAngBinCentres)
        ang = ang*np.pi/180
    ...
    def getOffsetRIndex(self, start, end):
        ...
        o = np.array([0,0])
        originOffset = np.linalg.norm(np.cross(end-start, start-o))/np.linalg.norm(end-start)
        
        offsetIndex = 2
        if originOffset < 1./3.:
            offsetIndex = 0
        elif originOffset < 2./3.:
            offsetIndex = 1
    ...
    def getStdv(self, offsetDIndex): # NO LONGER USED TO AVOID FUNCTION CALL OVERHEAD
        ...
        '''
        tStdv = np.random.choice(self.linStdvBinCentres, 1, p=self.linSpreadDist[offsetDIndex])[0]
        return tStdv
    ...
    def getLinPos(self, offsetRIndex): # NO LONGER USED TO AVOID FUNCTION CALL OVERHEAD
        ...
        '''
        lPos = self.smear(np.random.choice(self.linLBinCentres,
                                           1, p=self.linPointDist[offsetRIndex])[0], self.linLBinCentres)
        return lPos
    ...
    def getDevAng(self, z, s):
        ...
        if s == 0:
            ...
            devSampled = np.random.choice(self.angDevBinCentres, 1, p=self.angDevDist[zBin][s-1])[0]
    ...
    def getIncDirNear(self, hitPos, dist, angDev):
        ...
        directDir = direct/np.sqrt(direct.dot(direct)) # Get the direct line direction
        
        zenith = np.arccos(directDir[2])
        newZenith = ((zenith*180/np.pi) + angDev)*np.pi/180 # Determine the new zenith
        
        newZ = np.cos(newZenith)
        xyScale = np.sin(newZenith)/np.sin(zenith) # Scale x and y to z so that norm is 1
    ...
    def getSingleInc(self):
        ...
        ang = self.smear(np.random.choice(self.singleIncAngBinCentres,
                                          1, p=self.singleIncDist)[0], self.singleIncAngBinCentres)
        ang = ang * np.pi / 180
        ...
        incDir = np.array([inc[0]*np.cos(az) - inc[1]*np.sin(az), inc[0]*np.sin(az) + inc[1]*np.cos(az), inc[2]])
