...
class LeptonWeighter(icetray.I3ConditionalModule):
    ...
    def __init__(self, context):
        ...
        self.AddParameter("fluxIndex", "Index for flux parameteriazation", 1.0)
        self.AddParameter("fluxScale", "Scale for flux parameterization", 10**5)
        self.AddParameter("injectionRadius", " Radius of injection", 500.0)
        self.AddParameter(
            "computetaueffective", "Compue the tau effective weight for muons", False
        )
        self.AddParameter(
            "nuSQuIDS_WeightTables",
            "Tables for Earth survival weights",
            os.getenv("PONESRCDIR") + "/data/nsq_allneu_propagation_weight_gamma2.h5",
        )
    ...
    def GetnuSQUiDSIndex(self, primary_type):
        ...
        antiindex = 0
        if (
            primary_type == LW.ParticleType.NuMu
            or primary_type == LW.ParticleType.NuMuBar
        ):
            typeindex = 1
        elif (
            primary_type == LW.ParticleType.NuTau
            or primary_type == LW.ParticleType.NuTauBar
        ):
            typeindex = 2
        if (
            primary_type == LW.ParticleType.NuEBar
            or primary_type == LW.ParticleType.NuMuBar
            or primary_type == LW.ParticleType.NuTauBar
        ):
            antiindex = 1
    ...
    def DAQ(self, frame):
        ...
        if self.computetaueffective and (
            LWevent.primary_type == LW.ParticleType.NuMu
            or LWevent.primary_type == LW.ParticleType.NuMuBar
        ):
        ...
        if cos_zen < self.cosmax and cos_zen > self.cosmin:
            if nusq_energy < self.Emax and nusq_energy > self.Emin:
                survival_weight = self.nsq_atm.EvalFlavor(
                    typeindex, cos_zen, nusq_energy, antiindex
                ) / self.Eflux(nusq_energy)
            ...
            survival_weight = self.nsq_atm.EvalFlavor(
                typeindex, 0.0, nusq_energy, antiindex
            ) / self.Eflux(nusq_energy)
        else:
            # kill event
            survival_weight = 0.0
        ...
        flux = (
            self.fluxScale * (neutrino.energy * self.units.GeV) ** -self.fluxIndex
            + self.fluxConst
        )
        ...
        frame["LeptonInjection_oneweight_taueff"] = dataclasses.I3Double(
            weightone_taueff
        )
