...
parser.add_argument("-s", "--spacing",type = float, default = 80.0, help="Spacing for strings.")
...
def generateGeometry():
    ...
    offset = 0.0
    anglediff = np.pi*(2./5)
    neighbourangles = [offset, anglediff+offset,2.0*anglediff+offset, 3.0*anglediff+offset, 4.0*anglediff+offset]
    stringposx = [0.0,np.cos(neighbourangles[0]),np.cos(neighbourangles[1]),np.cos(neighbourangles[2]),np.cos(neighbourangles[3]),np.cos(neighbourangles[4])]
    stringposy =[0.0,np.sin(neighbourangles[0]),np.sin(neighbourangles[1]),np.sin(neighbourangles[2]),np.sin(neighbourangles[3]),np.sin(neighbourangles[4])]
