...
parser.add_argument(
    "-s", "--spacing", type=float, default=80.0, help="Spacing for strings in cluser."
)
parser.add_argument(
    "-l",
    "--clusterspacing",
    type=float,
    default=400.0,
    help="Spacing for strings in cluser.",
)
parser.add_argument(
    "-n", "--nstring", type=int, default=10, help="Number of strings per cluster."
)
parser.add_argument(
    "-c", "--nclusters", type=int, default=7, help="Number of clusters."
)
...
outfileName = (
    "PONE_" + str(args.nstring) + "String_" + str(args.nclusters) + "Cluster.i3.gz"
)
...
def generateGeometry():
    ...
    for i in range(len(clusterposx)):
        for j in range(len(stringposx)):
            if i == 0:
                FinalStringx.append(
                    stringposx[j] * spacing + clusterposx[i] * clusterspacing
                )
                FinalStringy.append(
                    stringposy[j] * spacing + clusterposy[i] * clusterspacing
                )
            else:
                FinalStringx.append(
                    np.cos(clustertheta[i]) * stringposx[j] * spacing
                    - np.sin(clustertheta[i]) * stringposy[j] * spacing
                    + clusterposx[i] * clusterspacing
                )
                FinalStringy.append(
                    np.sin(clustertheta[i]) * stringposx[j] * spacing
                    + np.cos(clustertheta[i]) * stringposy[j] * spacing
                    + clusterposy[i] * clusterspacing
                )
    ...
    for i in range(len(FinalStringx)):
        for m in range(domsPerString):
            ...
            omGeometry.position = dataclasses.I3Position(
                FinalStringx[i], FinalStringy[i], depth[m]
            )
