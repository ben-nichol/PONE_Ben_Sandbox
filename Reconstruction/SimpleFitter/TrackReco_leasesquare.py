...
def MinimizerFunctor(data, domsUsed, _minpos):
    ...
    def minimizerFunction(vx, vy, vz, theta, phi):
        direct = dataclasses.I3Direction(
        ...
        T0 = (
            -np.sqrt(
                (minpos.x - vertex.x) ** 2.0
                + (minpos.y - vertex.y) ** 2.0
                + (minpos.z - vertex.z) ** 2.0
            )
            / c_ngroup
        )
        ...
        for dom in deltaT.keys():
            ...
            vertex_to_dom = dataclasses.I3Position(
                dompos.x - vertex.x, dompos.y - vertex.y, dompos.z - vertex.z
            )
            dc = (
                vertex_to_dom.x * direct.x
                + vertex_to_dom.y * direct.y
                + vertex_to_dom.z * direct.z
            )
            l_c = dataclasses.I3Position(
                vertex_to_dom.x - dc * direct.x,
                vertex_to_dom.y - dc * direct.y,
                vertex_to_dom.z - dc * direct.z,
            )
            l_photon = l_c.r / sintheta
            l_emmission = dc - l_c.r / tantheta

            np.sqrt(
                (dompos.x - vertex.x) ** 2.0
                + (dompos.y - vertex.y) ** 2.0
                + (dompos.z - vertex.z) ** 2.0
            )
            for delt in deltaT[dom]:
                leastsquare += (
                    delt[1]
                    * (delt[0] - T0 - l_emmission / c - l_photon / c_ngroup) ** 2.0
                )
...
class TrackReco_leastsquare(icetray.I3ConditionalModule):
    def __init__(self, context):
        ...
        self.AddParameter("vertexRad", "The Radius to readjust the vertex to", 200.0)
    ...
    def Physics(self, frame):
        ...
        for i in range(3):
            ...
            m.limits["vx"] = (minpos.x - 200.0, minpos.x + 200.0)
            m.limits["vy"] = (minpos.y - 200.0, minpos.y + 200.0)
            m.limits["vz"] = (minpos.z - 200.0, minpos.z + 200.0)
