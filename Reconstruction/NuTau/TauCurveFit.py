...
from Utility.NuTauPeakShapes import (
    likelihood_ratio_doublePeak,
    likelihood_ratio_biGauss,
    biGauss,
    double_peak,
)
from Utility.NuTauPeakShapes import (
    log_likelihood_expGauss,
    log_likelihood_expDoublePeak,
    expGauss,
    expDoublePeak,
)
import scipy, csv
...
class curveFit(icetray.I3ConditionalModule):
    ...
    def __init__(self, context):
        ...
        self.AddParameter("omgeo", "geometry map given for the analysis", "omgeo")

        self.AddParameter(
            "InputMCPETree", "Input MCPETree name for analysis", "MCPESeriesMap"
        )

        self.AddParameter(
            "OutputMCPETree", "Output MCPETree name", "CurvefitParameters"
        )

        self.AddParameter("HitsInDOMsCut", "Cut in the num fits in DOMS", 200)

        self.AddParameter("FrameList", "List of frame numbers to debug", [])

        self.AddParameter("StringList", "List of string numbers to debug", [])

        self.AddParameter("DOMList", "List of DOM numbers to debug", [])
        ...
        for omkey in self.nopmtkeys:
            ...
            for ipmt in range(self.npmt):
                ...
                if omkey_wpmt in waveformmapseries.keys():
                    for wave in waveformmapseries[omkey_wpmt]:
                        ...
                        maxtime = max(
                            maxtime, wave.time + wave.binwidth * len(wave.waveform)
                        )
            ...
            mean_charge = sum(
                [
                    wavearray[i]
                    for i in range(
                        max(0, int(mean - 50.0 / binwidth)),
                        max(len(wavearray), int(mean + 50.0 / binwidth)),
                    )
                ]
            )
            ...
            bin_center_bool = (bin_centers >= min(bin_centers_select) - 6) & (
                bin_centers <= max(bin_centers_select) + 6
            )
            ...
            soln_biGauss = 0
            bnds_biGauss = [
                [min(bin_centers), maxBinCenter],
                [1, time_window],  # Let the width be negative
                [1, 20],  # Restrict k to be positive, but only up to 20
                [0.1, 2 * max(entries_in_bins)],
            ]  # Don't restrict the amplitude, it will vary greatly with K

            for iK in init_k:
                for iwid in init_wid:
                    initial_biGauss = np.array(
                        [final_mean, iwid, iK, max(entries_in_bins)]
                    )

                    # Single Peak
                    nll = lambda *args: log_likelihood_biGauss(*args)

                    soln_single = minimize(
                        log_likelihood_expGauss,
                        initial_biGauss,
            ...
            # Double Peak
            peak_time_boundary = final_mean - 6.0
            bnds_doublePeak = [
                [min(bin_centers), peak_time_boundary],
                bnds_biGauss[1],
                bnds_biGauss[2],
                bnds_biGauss[3],
            ...
            """
            LR_biGauss = likelihood_ratio_biGauss(
                bin_centers,
                entries_in_bins,
                soln_biGauss.x[0],
                soln_biGauss.x[1],
            ...
            LR_doublePeak = likelihood_ratio_doublePeak(
                bin_centers,
                entries_in_bins,
                soln_doublePeak.x[0],
                soln_doublePeak.x[1],
            ...
            biGauss_valuesMap.update(
                {omkey: dataclasses.I3VectorDouble(biGauss_values)}
            )
            doublePeak_valuesMap.update(
                {omkey: dataclasses.I3VectorDouble(doublePeak_values)}
            )
