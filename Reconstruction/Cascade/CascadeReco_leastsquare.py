...
def DirectionFit(data, domsUsed, _vertex):
    ...
    def likelihoodFunction(theta, phi):
        ...
        for dom in distance.keys():
            ...
            vert_to_dom = dataclasses.I3Direction(
                dompos.x - vertex.x, dompos.y - vertex.y, dompos.z - vertex.z
            )
            dot = (
                vert_to_dom.x * direction.x
                + vert_to_dom.y * direction.y
                + vert_to_dom.z * direction.z
            )
            for dist in distance[dom]:
...
class CascadeReco(icetray.I3ConditionalModule):
    ...
    def Physics(self, frame):
        ...
        for i in range(3):
            ...
            m = Minuit(func, vx=vertex.x, vy=vertex.y, vz=vertex.z)
            ...
            for dom in data.keys():
                ...
                if Vertex_to_dom > 0.0:
                    _direct = [
                        (dompos.x - q.x) / Vertex_to_dom,
                        (dompos.y - q.y) / Vertex_to_dom,
                        (dompos.z - q.z) / Vertex_to_dom,
                    ]
            ...
            m2 = Minuit(func2, theta=seeddir.theta, phi=seeddir.phi)
            ...
            direction = dataclasses.I3Direction(
                np.sin(res2["theta"] * np.cos(res2["phi"])),
                np.sin(res2["theta"]) * np.sin(res2["phi"]),
                np.cos(res2["theta"]),
            )
