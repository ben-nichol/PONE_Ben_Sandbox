...
class TimingFit(icetray.I3Module):
    def gamma_t_only(module, direct, origin):
        ...
        pos = lambda t: dataclasses.I3Position(
            origin.x + c * t * direct.x,
            origin.y + c * t * direct.y,
            origin.z + c * t * direct.z,
        )
        ...
        d_gamma = (
            n
            / np.sqrt(n**2 - 1)
            * np.sqrt(d_close**2 + (module.z - z_close) ** 2 * (1.0 - direct.z**2))
        )
        t_gamma = (
    ...
    def __init__(self, context):
        ...
        self.AddParameter("LinefitName", "Name of the Linefit result", "linefit")
        self.AddParameter("FixVertexRadius", "Fix the vertex of the fitter", True)
        self.AddParameter("vertRadius", "radius to restrict the vertext to", 200.0)
    ...
    def Fit(self, pulse_time, pulse_pos, quality_function, guesstrack):
        def data_loop(phi, cos_theta, cos_o_theta, o_phi, eventtime):
            ...
            for t1, pos in zip(pulse_time, pulse_pos):
                t2 = TimingFit.gamma_t_only(
                    module=pos,
                    direct=dataclasses.I3Position(
                        np.sqrt(1.0 - cos_theta**2.0) * np.cos(phi),
                        np.sqrt(1.0 - cos_theta**2.0) * np.sin(phi),
                        cos_theta,
                    ),
                    origin=dataclasses.I3Position(
                        self.vertexRad
                        * np.sqrt(1.0 - cos_o_theta**2.0)
                        * np.cos(o_phi),
                        self.vertexRad
                        * np.sqrt(1.0 - cos_o_theta**2.0)
                        * np.sin(o_phi),
                        self.vertexRad * cos_o_theta,
                    ),
                )
        ...
        m.limits["eventtime"] = (-1000.0, 2000.0)
        m.errors["phi"] = 1.0
        m.errors["cos_theta"] = 0.1
        m.errors["cos_o_theta"] = 0.1
        m.errors["o_phi"] = 1.0
        m.errors["eventtime"] = 50.0 / (0.299 / 1.35)
        ...
        fit.dir = dataclasses.I3Direction(
            np.sqrt(1.0 - res["cos_theta"] ** 2.0) * np.cos(res["phi"]),
            np.sqrt(1.0 - res["cos_theta"] ** 2.0) * np.sin(res["phi"]),
            res["cos_theta"],
        )

        fit.pos = dataclasses.I3Position(
            self.vertexRad
            * np.sqrt(1.0 - res["cos_o_theta"] ** 2.0)
            * np.cos(res["o_phi"]),
            self.vertexRad
            * np.sqrt(1.0 - res["cos_o_theta"] ** 2.0)
            * np.sin(res["o_phi"]),
            self.vertexRad * res["cos_o_theta"],
        )
    ...
    def Physics(self, frame):
        ...
        for guess, quality_function, name in [
            ...
            try:
                guesstrack = frame[guess][0]
            except TypeError:
                guesstrack = frame[guess]
