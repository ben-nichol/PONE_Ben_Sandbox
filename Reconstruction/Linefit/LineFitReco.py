...
class LineFitReco(icetray.I3ConditionalModule):
    def __init__(self, context):
        ...
        self.AddParameter("vertRadius", "radius to readust the vertext to", 200.0)
    ...
    def Physics(self, frame):
        ...
        weighted_time = np.sum(np.array(t) * np.array(charge)) / np.sum(
            np.array(charge)
        )
        ...
        T0 = (
            weighted_time
            - np.sqrt(
                (vertex.x - weighted_x) ** 2.0
                + (vertex.y - weighted_y) ** 2.0
                + (vertex.z - weighted_z) ** 2.0
            )
            / 0.299
        )
