...
class K40Noise(icetray.I3ConditionalModule):
    def __init__(self, context):
        ...
        self.AddParameter(
            "charFile",
            ".pkl file that hold the noise characterization",
            os.getenv("PONESRCDIR") + "/data/NoiseCharacterization.pkl",
        )
        self.AddParameter(
            "inputTreeName",
            "Name of the input tree that holds physics data to span the noise generation over",
            "I3Photon",
        )
        self.AddParameter(
            "outputTreeName",
            "Name of the tree that the noise photons will be written to",
            "K40",
        )
        self.AddParameter(
            "startPadding", "Time padding before the first physics photon [ns]", 1000
        )
        self.AddParameter(
            "endPadding", "Time padding after the last physics photon [ns]", 10000
        )
        self.AddParameter(
            "skipSingles",
            "Choose whether to ignore events that only give 1 photon",
            False,
        )
        self.AddParameter(
            "pmtHitsOnly",
            "Choose whether to only write photons that landed on a PMT",
            False,
        )
        self.AddParameter(
            "skipOneFold",
            "Choose whether to ignore events that only hit 1 PMT. If this is set to True, pmtHitsOnly must also bet set to True.",
            False,
        )
    ...
    def Configure(self):
        ...
        if self.skipOneFold == True:
            if self.pmtHitsOnly == False:
                raise Exception(
                    "pmtHitsOnly must be set to True if skipOneFold is True"
                )
        ...
        thetaVals = list(np.pi / 180 * np.array([65.0, 32.5, 115.0, 147.5]))
        phiVals1 = list(np.pi / 180 * np.array([360.0, 270.0, 180.0, 90.0]))
        phiVals2 = list(np.pi / 180 * np.array([315.0, 225.0, 135.0, 45.0]))

        # Theta and phi arrays of coordinates for each of the PMTs in the DOM
        thetaList = np.array([0.0 for i in np.arange(16)])
        phiList = np.array([0.0 for i in np.arange(16)])
    ...
    def HitCheck(self, photonPosition):
        ...
        pmtrad = (
            55.0
        ) * 1e-3  # Effective PMT area is up to the size of the gel pad [m]

        angles = self.pmtMatrix.dot(
            np.array([photonPosition[0], photonPosition[1], photonPosition[2]])
            / np.linalg.norm(photonPosition)
        )
        pmtHit = np.where(
            np.logical_and(
                self.domR * np.sin(np.arccos(angles)) <= (pmtrad), angles >= 0
            )
        )[0]
        ...
        if len(pmtHit) == 1:  # One PMT hit, figure out which
            ...
            raise IndexError(
                "Photon is hitting two (or more) PMTs at the same time!",
                pmtHit,
                photonPosition,
                angles[pmtHit],
            )
    ...
    def GenerateFrameNoise(self, duration, timeOffset):
        ...
        for t in np.arange(numEvents):
            ...
            rotAng = 2 * np.pi - np.arccos(
                startDirProj.dot(decayDirProj)
            )  # Angle in radians
            ...
            if numPs < 2:
                ...
                # Create the photon
                photons[photonNum] = makePhoton(
                    hitPosition, hitDirection, frameTimes[t], wl, decayNum
                )
                if self.pmtHitsOnly:
            ...
            tStdv = np.random.choice(
                k40.linStdvBinCentres, 1, p=k40.linSpreadDist[offsetDIndex]
            )[0]
            ...
            for p in np.arange(numPs):
                ...
                lPos = k40.smear(
                    np.random.choice(
                        k40.linLBinCentres, 1, p=k40.linPointDist[offsetRIndex]
                    )[0],
                    k40.linLBinCentres,
                )
                ...
                if np.linalg.norm(hitPos) > 1:
                    hitPos = (
                        0.99 * hitPos / np.linalg.norm(hitPos)
                    )  # 0.99 so extending to 3D sqrt doesn't give err

                # Get the z coordinate based on x and y
                if hitPos[0] ** 2 - hitPos[1] ** 2 >= 1.0:
                    hitZ = 0.0
                else:
                    hitZ = np.sqrt(1.0 - hitPos[0] ** 2 - hitPos[1] ** 2)
                ...
                if firstPhoton:
                    ...
                    hTime = k40.getNextTime(
                        frameTimes[t], numScatters, timeOffset, timeOffset + duration
                    )

                # Create a photon and add it to the list
                photons[photonNum] = makePhoton(
                    hitPosition, hitDirection, hTime, wl, decayNum
                )
                if self.pmtHitsOnly:
                    hitPMTs[photonNum] = self.HitCheck(hitPosition)
                photonNum += 1
